...
| `uav_tree_crown` | object detection | YOLOv8 | runtime supported once model branch lands | 1 |
...
UAV crown runtime wiring is ready on `dev`: once the UAV model branch lands its
`inference_config.yaml`, `metrics.json`, and ignored
`runs/uav_yolo_tree_crown_detector_v1/weights/best.pt`, AI Engine can register a
real YOLO predictor for `image_role=uav_tile` through

...
fn run_managed(run_cfg: RunConfig) -> Result<(), String> {
    ...
    println!("[{}][gateway] Managed mode started. target is cached in state profile.", ts());
    println!("[{}][gateway] Recursively scanning serial ports and baud rates...", ts());
        ...
        for port in ports {
            ...
            if let Some(baud) = discovery.detected_baud {
                ...
                println!(
                    ...
                    ts(), device.port, device.baud, device.sensors
                );
...
fn reap_finished_sessions(running: &mut HashMap<String, JoinHandle<()>>) {
    let finished: Vec<String> = running
        ...
        .filter_map(|(port, handle)| {
            if handle.is_finished() { Some(port.clone()) } else { None }
        })
        .collect();
    for port in finished {
        if let Some(handle) = running.remove(&port) {
            match handle.join() {
                Ok(()) => println!("[{}][gateway] Session finished, back to recursive search: {port}", ts()),
                Err(_) => eprintln!("[{}][gateway] Session panic, back to recursive search: {port}", ts()),
            }
        }
    }
}

fn discover_device_for_port(port: &str, shared: &SharedContext) -> Result<PortDiscoveryOutcome, String> {
    ...
    for &baud in &shared.run_cfg.baud_list {
        ...
        if !result.sample_lines.is_empty() {
            println!("[{}][gateway] Discovery sample {port}@{baud}: {:?}", ts(), result.sample_lines);
        }
...
fn get_or_create_device_id(port: &str, state_dir: &Path, device_index: &Arc<Mutex<DeviceIndexStore>>) -> Result<(String, bool), String> {
    let mut index_guard = device_index.lock().map_err(|_| "Device index lock poisoned".to_string())?;
    if let Some(existing) = index_guard.port_to_device_id.get(port) {
        return Ok((existing.clone(), false));
    }
    let created = generate_device_id(port);
    index_guard.port_to_device_id.insert(port.to_string(), created.clone());
...
fn run_device_session_loop(device: DiscoveredDevice, shared: SharedContext) -> Result<(), String> {
    loop {
        ...
        let socket = UdpSocket::bind("0.0.0.0:0").map_err(|e| format!("Failed to bind local UDP socket: {e}"))?;
        socket.set_read_timeout(Some(shared.run_cfg.ack_timeout)).map_err(|e| format!("Failed to set ACK timeout: {e}"))?;
        let register_result = register_device(&socket, &device, &shared);
        if let Err(err) = register_result {
            eprintln!("[{}][gateway] Registration failed for {}: {err}", ts(), device.device_id);
            return Ok(());
        }
        loop {
            let event = match source.next_event() {
                Ok(v) => v,
                Err(err) => {
                    eprintln!("[{}][gateway] Source read error on {}: {err}", ts(), source.name());
                    break;
                }
            };
            let target = {
                let guard = shared.profile.lock().map_err(|_| "Profile lock poisoned".to_string())?;
                guard.cloud_target.clone()
            };
            let payload = if event.sensor_id == RESERVED_IMAGE_SENSOR_ID || event.feature == RESERVED_IMAGE_FEATURE {
            ...
            socket.send_to(payload.as_bytes(), &target).map_err(|e| format!("Failed to send payload to {target}: {e}"))?;
            ...
            let (ack_size, ack_peer) = match socket.recv_from(&mut ack_buf) {
                ...
                Err(err) if err.kind() == ErrorKind::TimedOut || err.kind() == ErrorKind::WouldBlock => {
                    eprintln!("[{}][gateway] ACK timeout from {target}, keeping session alive for {}", ts(), device.device_id);
                    continue;
                }
                Err(err) => {
            ...
            if ack == "ack:unregistered" || ack == "ack:token_invalid" {
                println!("[{}][gateway] {} requires re-register (ack={}), re-registering...", ts(), device.device_id, ack);
                register_device(&socket, &device, &shared)?;
                continue;
            }
            println!("[{}][gateway] {} -> {} payload=\"{}\" ACK {} from {}", ts(), event.feature, target, payload, ack, ack_peer);
        }
        println!("[{}][gateway] Session dropped for {}, back to discovery.", ts(), device.port);
        return Ok(());
    }
}
...
fn register_device(socket: &UdpSocket, device: &DiscoveredDevice, shared: &SharedContext) -> Result<(), String> {
    loop {
        ...
        let payload = RegisterPayload {
            ...
            location, crop_type, farm_note,
            sensors: device.sensors.clone(),
            feature_mapping: device.feature_mapping.clone(),
            token,
        };
        let packet = build_register_packet(&payload)?;
        socket.send_to(packet.as_bytes(), &target).map_err(|e| format!("Failed to send register packet: {e}"))?;
        ...
        match ack.as_str() {
            "ack:register_ok" => {
                println!("[{}][gateway] register ok: device_id={} sensors={:?}", ts(), device.device_id, device.sensors);
                return Ok(());
            }
            "ack:token_invalid" => {
            ...
            "ack:register_conflict" => {
                return Err(format!("Cloud reported register conflict for device_id {}", device.device_id));
            }
            other => {
...
fn run_diag(cfg: DiagConfig) -> Result<(), String> {
    ...
    println!("[{}][gateway][diag] feature mapping loaded: {:?}", ts(), feature_map.mappings);
    let ports = list_serial_ports()?;
    if ports.is_empty() {
    ...
    for port in ports {
        ...
        for baud in &cfg.baud_list {
            match discover_on_port(&port, *baud, cfg.scan_window) {
                Ok(found) => {
                    if found.known_sensors.is_empty() && found.unknown_features.is_empty() { continue; }
                    println!("  baud={} known={:?} unknown={:?} sample={:?}", baud, found.known_sensors, found.unknown_features, found.sample_lines);
                }
                Err(err) => { println!("  baud={} open/read failed: {}", baud, err); }
            }
        }
    }
    Ok(())
...
fn prompt_initial_profile(target_override: Option<&str>) -> Result<GatewayProfile, String> {
    ...
    let location = prompt_line("Farm location (self-describe):", Some("unknown_location"), &Arc::new(Mutex::new(())))?;
...
fn prompt_line(message: &str, default: Option<&str>, prompt_lock: &Arc<Mutex<()>>) -> Result<String, String> {
    ...
    if let Some(default_value) = default {
        if !default_value.is_empty() { print!(" [{}]", default_value); }
    }
    print!(" ");
    io::stdout().flush().map_err(|e| format!("Failed to flush stdout: {e}"))?;
    let mut raw = Vec::new();
    io::stdin().lock().read_until(b'\n', &mut raw).map_err(|e| format!("Failed to read stdin: {e}"))?;
    let decoded = decode_input_lossy(&raw);
    let trimmed = decoded.trim();
    if trimmed.is_empty() { return Ok(default.unwrap_or("").to_string()); }
...
fn decode_input_lossy(raw: &[u8]) -> String {
    ...
    if raw.len() >= 2 && raw.chunks_exact(2).all(|chunk| chunk[1] == 0) {
        ...
        for chunk in raw.chunks_exact(2) { utf16.push(u16::from_le_bytes([chunk[0], chunk[1]])); }
...
fn generate_device_id(port: &str) -> String {
    let ts_val = SystemTime::now().duration_since(UNIX_EPOCH).map(|d| d.as_secs()).unwrap_or(0);
    let cleaned: String = port.chars().map(|c| {
...
pub(crate) fn send_internal_success_probe(target: &str, ack_timeout: Duration) -> Result<(), String> {
    ...
    socket.set_read_timeout(Some(ack_timeout)).map_err(|e| format!("Failed to set timeout: {e}"))?;
    socket.send_to(DEFAULT_PAYLOAD_SUCCESS.as_bytes(), target).map_err(|e| format!("Failed to send success probe: {e}"))?;
    let mut buf = [0_u8; 512];
    let (size, _) = socket.recv_from(&mut buf).map_err(|e| match e.kind() {
        ErrorKind::TimedOut | ErrorKind::WouldBlock => "Success probe timeout while waiting ACK".to_string(),

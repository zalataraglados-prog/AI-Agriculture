...
class UAVYoloPredictor(BasePredictor):
    crop = "oil_palm"
    task = "uav_tree_crown"
    mode = "real"
    ...
    def __init__(
        ...
        *,
        ...
        labels_path: str | Path = "models/oil_palm/uav_tree_crown/labels.json",
        config_path: str | Path | None = "models/oil_palm/uav_tree_crown/inference_config.yaml",
        ...
        self.model_version = str(
            self.config.get("model_version", "oil_palm_uav_yolo_tree_crown_detector_v1")
        )
        ...
        self.max_detections = int(self.config.get("max_detections", 1000))

        if not self.weights_path.exists():
            raise FileNotFoundError(f"UAV crown YOLO weights not found: {self.weights_path}")
        if not self.labels_path.exists():
            raise FileNotFoundError(f"UAV crown labels file not found: {self.labels_path}")

        try:
            from ultralytics import YOLO
        except Exception as exc:  # pragma: no cover - exercised when deps missing
            raise RuntimeError(
                "ultralytics is required for the real UAV crown YOLO predictor"
            ) from exc
    ...
    def predict(self, image_bytes: bytes, context: PredictorContext) -> dict[str, Any]:
        ...
        metadata = {
            ...
            "mock": False,
            "model_profile": "uav_yolo_tree_crown_detector",
            "candidate_type": "tree_crown",
            "detections_count": len(candidates),
            "review_status": "requires_human_confirmation",
            "coordinate_scope": "tile_normalized",
            "cloud_postprocess_required": [
                "tile_offset_to_global_coordinates",
                "cross_tile_nms",
                "human_confirmation",
                "tree_code_assignment",
            ],
    ...
    def _candidates_from_result(
        ...
        for idx, xyxy in enumerate(xyxy_values):
            ...
            candidates.append(
                {
                    "candidate_id": f"uav_crown_{idx + 1:03d}",
                    "label": label,
    ...
    def _load_config(path: Path | None) -> dict[str, Any]:
        ...
        if not isinstance(data, dict):
            raise ValueError(f"UAV crown inference config must be a mapping: {path}")
        return data
    ...
    def _load_labels(path: Path) -> list[str]:
        ...
        if not isinstance(labels, list) or not all(isinstance(v, str) for v in labels):
            raise ValueError(f"UAV crown labels must be a JSON list of strings: {path}")
        return labels


def build_uav_yolo_predictor_from_env() -> UAVYoloPredictor:
    weights_path = os.environ.get(
        "OIL_PALM_UAV_CROWN_MODEL_PATH",
        "models/oil_palm/uav_tree_crown/runs/uav_yolo_tree_crown_detector_v1/weights/best.pt",
    )
    labels_path = os.environ.get(
        "OIL_PALM_UAV_CROWN_LABELS_FILE",
        "models/oil_palm/uav_tree_crown/labels.json",
    )
    config_path = os.environ.get(
        "OIL_PALM_UAV_CROWN_CONFIG_FILE",
        "models/oil_palm/uav_tree_crown/inference_config.yaml",
    )
    threshold = os.environ.get(
        "OIL_PALM_UAV_CROWN_CONFIDENCE_THRESHOLD",
        os.environ.get("OIL_PALM_CONFIDENCE_THRESHOLD"),
    )
    parsed_threshold = float(threshold) if threshold not in (None, "") else None
    return UAVYoloPredictor(

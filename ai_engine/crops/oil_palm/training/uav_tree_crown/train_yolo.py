"""Train the UAV tree crown YOLO detector.

Heavy ML imports stay inside the execution path so tests can import this module
without installing the full training stack.
"""
...
DEFAULT_CONFIG = PROJECT_ROOT / "models" / "oil_palm" / "uav_tree_crown" / "training_config.example.yaml"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the oil palm UAV crown YOLO detector.")
    parser.add_argument(
        "--config",
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = {
        "task": config.get("task", "uav_tree_crown"),
        "model": model_name,
